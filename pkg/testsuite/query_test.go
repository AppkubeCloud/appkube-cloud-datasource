...
import (
	...
	"github.com/appkube/cloud-datasource/pkg/testsuite"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.skipGoldenCheck {
				...
				experimental.CheckGoldenJSONResponse(t, "testdata", strings.ReplaceAll(t.Name(), "TestInlineSources/", "inline/"), response, testsuite.UPDATE_GOLDEN_DATA)
			}
			if tt.test != nil {
				tt.test(t, frame)
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.skipGoldenCheck {
				require.NotNil(t, frame)
				response := &backend.DataResponse{Frames: data.Frames{frame}}
				experimental.CheckGoldenJSONResponse(t, "testdata", strings.ReplaceAll(t.Name(), "TestRemoteSources/", "remote/"), response, testsuite.UPDATE_GOLDEN_DATA)
			}
			if tt.test != nil {
				tt.test(t, frame)
			}
